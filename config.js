...
async function fetchAndDisplayWinnersForAdmin() {
    ...
    tbody.innerHTML = '';
    try {
        const response = await fetch('/api/admin/ganadores');
...
async function fetchAndDisplayParticipants() {
    ...
    try {
        const response = await fetch('/api/admin/participantes-activos');
...
async function handleFinalizarSorteo(sorteoId) {
    ...
    try {
        const response = await fetch('/api/admin/sorteos/finalizar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
        const response = await fetch('/api/admin/dashboard-avanzado');
